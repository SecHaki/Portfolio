---
layout: default
---

# 👋-Welcome to My Portfolio! 

## About Me  
I'm an aspiring cybersecurity professional pursuing a Bachelor of Science in Cybersecurity and Information Assurance at WGU, with hands-on experience as a Cybersecurity Systems Assistant. I specialize in threat detection, incident response, network security, and endpoint hardening, and I am passionate about applying practical security operations skills to protect organizations and users.


--- 

## 🎓-Education

### Bachelor of Science in Cybersecurity and Information Assurance (In Progress)  
**Western Governors University, Salt Lake City, UT**  
**Expected Graduation:** May 2026  
**Relevant Coursework:** Linux Foundations, Cryptography, Cloud Security, Cyber Defense (CySA+), Penetration Testing (Pentest+), Networks (Network+), Information Systems Security, Legal & Ethical Issues in Cybersecurity  

### Associate of Science in Cybersecurity  
**Pasadena City College – Pasadena, CA**  
**Graduation:** December 2024 | **GPA:** 3.6  
**Certificates of Achievement:** Cybersecurity, Systems & Network Administration, Help Desk  
**Relevant Coursework:** Network Security, System Administration, Cybersecurity Fundamentals, Ethical Hacking, Digital Forensics, SQL, Python

---

### Certifications & CTF's 
- **Certifications:**
  - GIAC-GSEC
  - CompTIA Network+
  - CompTIA A+
- **CTF's Rankings**
  - SentinelOne Purple AI ThreatOps Challenge: First Place
  - SentinelOne EDR ThreatOps Challenge: Third Place

<hr style="border: 1px solid #ccc; margin: 20px 0;">

<div style="display: flex; gap: 10px;">
  <a href="https://www.credly.com/badges/eda603c2-8af8-485f-84f9-8b2901f35842/public_url">
    <img src="Assets/Certification/giac-security-essentials-certification-gsec.png" style="width: 100px; height:100px;">
  </a> <!-- Closing the first <a> tag -->

<a href="https://www.credly.com/badges/16380ce9-0baa-4ab4-b71c-c41cefa8b4b2/public_url">
    <img src="Assets/Certification/comptia-network-ce-certification.1.png" style="width: 110px; height:110px;">
  </a> <!-- Closing the second <a> tag -->
  
  <a href="https://www.credly.com/badges/fd9bc18e-5a51-48fb-baec-c11ca84ffdda/public_url">
    <img src="Assets/Certification/comptia-a-ce-certification.1.png" style="width: 110px; height:110px;">
  </a> <!-- Closing the second <a> tag -->
  
  <a href="https://www.credly.com/badges/aac3635c-0a06-4482-a983-3b25dd7498df/public_url">
    <img src="Assets/Certification/sentinelone-purple-ai-threatops-challenge-first-pla.png" style="width: 100px; height:100px;">   
  </a> <!-- Closing the third <a> tag -->
  
  <a href="https://www.credly.com/badges/c44ded03-93b2-488c-aff1-d98813e7f614/public_url">
    <img src="Assets/Certification/sentinelone-edr-threatops-challenge-third-place.png" style="width: 100px; height:100px;">   
  </a> <!-- Closing the third <a> tag -->
</div>

<hr style="border: 1px solid #ccc; margin: 20px 0;">
---

# 💻 Showcasing my Homelab and Projects
## [🔎-Splunk Detection Queries](./projects/Splunk/Splunk-Library.html)
## [🌐-Network Lab](./projects/network-lab.html)
## [More Coming Soon]()

---

<h2>Skills and Tools</h2>

<table>
  <thead>
    <tr>
      <th>🤖 Technical Skills</th>
      <th>⚒️ Tools & Platforms</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>🐧 Operating Systems</td>
      <td>Windows 10/11, Linux (basic), macOS, Chrome OS</td>
    </tr>
    <tr>
      <td>👥 Active Directory & Identity Management</td>
      <td>Active Directory, Azure AD, Conditional Access Policies, PowerShell, Active Roles</td>
    </tr>
    <tr>
      <td>🛡️ Endpoint Security</td>
      <td>SentinelOne, Malwarebytes, Carbon Black</td>
    </tr>
    <tr>
      <td>⚔️ Incident Response & Monitoring</td>
      <td>Splunk, Microsoft Defender, ProofPoint-TRAP, ANY.RUN, VirusTotal, URLScan.io</td>
    </tr>
    <tr>
      <td>🚀 Security Automation</td>
      <td>PowerShell, Intune</td>
    </tr>
    <tr>
      <td>🧱 Configuration Hardening & Compliance</td>
      <td>CIS Benchmarks, Windows Security Baselines, CJIS Compliance</td>
    </tr>
    <tr>
      <td>📨 Security Awareness</td>
      <td>Proofpoint Awareness Platform, Phishing Simulations</td>
    </tr>
    <tr>
      <td>📃 Scripting & Reporting</td>
      <td>PowerShell, SQL, Excel (AD exports), SharePoint</td>
    </tr>
    <tr>
      <td>🔍 Threat Detection & Hunting</td>
      <td>Splunk, SentinelOne, YARA, Shodan</td>
    </tr>
    <tr>
      <td>📡 Network & Device Management</td>
      <td>Smart UPS (APC), Intune, SentinelOne Dynamic Groups</td>
    </tr>
    <tr>
      <td>📥 Security Dashboards</td>
      <td>Splunk (custom dashboards), SentinelOne Policy Matrix</td>
    </tr>
    <tr>
      <td>🚩 Threat Intelligence Frameworks</td>
      <td>MITRE ATT&CK, NIST Cybersecurity Framework</td>
    </tr>
    <tr>
      <td>🔐 Password & Account Management</td>
      <td>Self-hosted On-Prem Password Manager, Active Directory Audits</td>
    </tr>
  </tbody>
</table>

### 🧠-Soft Skills

- Effective written and verbal communication across technical and non-technical audiences  
- Critical thinking and analytical problem-solving for complex cybersecurity tasks  
- Strong attention to detail in documentation, reporting, and process execution  
- Time management and ability to meet tight deadlines in high-pressure environments  
- Adaptability in dynamic, fast-paced IT and cybersecurity settings  
- Collaboration across cross-functional teams to achieve operational goals  
- Self-driven continuous learner, proactively improving skills and knowledge  
- Leadership and initiative in small-scale projects, training, and awareness campaigns

---
